drop schema spm_project;
create schema spm_project;
use spm_project;
-- role table (user roles, eg staff ,HR, admin)
create table role(
...
-- Insertion of Admin / User / Manager / HR roles
...
('1', 'Introduction to Python', 'This is an introduction course for people who are interested in programming!', 'Active', 'Internal', 'Technical'),
('2', 'Introduction to Java', 'This is an introduction course for people who are interested in Object Oriented Programming!', 'Active', 'Internal', 'Technical'),
('3', 'Introduction to SQL', 'This is an introduction course for people who are interested in SQL!', 'Active', 'Internal', 'Technical'),
('4', 'Introduction to Tableau', 'This is an introduction course for people who are interested in Tableau!', 'Active', 'Internal', 'Technical'),
...
-- job_roles table (not the same as role mentioned above)
create table job_role(
Job_Role_ID int not null AUTO_INCREMENT, 
Job_Role_Name varchar(50) not null,
Job_Role_desc varchar(255) not null,
Job_Role_Deleted TINYINT(1) default 0,
constraint job_roles_pk Primary key (Job_Role_ID)
);


insert into job_role (Job_Role_ID, Job_Role_Name, Job_Role_desc)
values (1, 'Engineer', 'The building block of any company. An engineer helps to facilitate the smooth operation of the company on a daily basis.');

insert into job_role (Job_Role_ID, Job_Role_Name, Job_Role_desc)
values (2, 'Business Analyst', 'An integral role in recent years. A Business Analyst helps the company to identify shortcomings and find potential money making opportunities.');

insert into job_role (Job_Role_ID, Job_Role_Name, Job_Role_desc)
values (3, 'Software Developer', 'It is the one who is writing the code for this right now.');

insert into job_role (Job_Role_ID, Job_Role_Name, Job_Role_desc)
values (4, 'Product Manager', 'Knows Agile');

insert into job_role (Job_Role_ID, Job_Role_Name, Job_Role_desc)
values (5, 'Data Analyst', 'Analyses and visualizes data');
...
Skill_Deleted TINYINT(1) default 0,
constraint job_skills_pk primary key (Skill_ID)
);

insert into job_skill(Skill_ID, Skill_Name)
values (1, 'Python');
...
-- job_roles_skills table
create table job_role_skill(
Skill_ID int not null,
Job_Role_ID int not null, 
constraint job_role_skill_pk Primary key (Skill_ID,Job_Role_ID),
constraint job_role_skill_fk1 foreign key (Job_Role_ID) references job_role(Job_Role_ID),
constraint job_role_skill_fk2 foreign key (Skill_ID) references job_skill(Skill_ID)
);

-- Insertion of skills for job roles
insert into job_role_skill(Skill_ID, Job_Role_ID)
values (1, 2), (4, 2), (2, 3), (1, 3), (1, 5), (3, 5), (4, 4), (5, 4), (6, 4), (7, 5);
...
-- job_course_skills table 
create table course_skill(
Course_ID varchar(20) not null,
Skill_ID int not null,
constraint course_skill_pk primary key (Course_ID,SKill_ID),
constraint course_skill_fk1 foreign key (Course_ID) references course(Course_ID),
constraint course_skill_fk2 foreign key (Skill_ID) references job_skill(Skill_ID)
);

insert into course_skill(Course_ID, Skill_ID)
values (1,1), (1,5), (1,6), (1,7), (2,2), (3,3), (3,4), (3,7), ("HRD001", 5), ("MGT001", 4), ("MGT001", 6), ("MGT004", 4), ("MGT007", 4), ("SAL004", 5), ("tch008", 7), ("tch009", 7), ("tch018", 6), ("tch019", 6);
...
-- learning journey table (reference job_role, skill, staff )
...
-- constraint learning_journey_fk1  foreign key (Course_ID, Skill_ID) references course_skill(Course_ID,Skill_ID),
-- constraint learning_journey_fk2  foreign key (Skill_ID,Job_Role_ID) references job_role_skill(Skill_ID,Job_Role_ID),
constraint learning_journey_fk foreign key (Staff_ID) references staff(Staff_ID)
);


insert into learning_journey(Job_Role_ID, Courses, Staff_ID)
values (4,'["SAL004","HRD001","1"]',130002), (2,'["1"]',140001), (4,'["SAL004","HRD001"]',140001);
